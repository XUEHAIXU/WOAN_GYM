...
from legged_gym.envs.WOAN4310_MoB.WOAN4310_Trot.WOAN4310_config import WOAN4310_Cfg_Yu
...
class WOAN4310_Robot(BaseTask):
    def __init__(self, cfg: WOAN4310_Cfg_Yu, sim_params, physics_engine, sim_device, headless):
    ...
    def post_physics_step(self):
        ...
        self.base_euler_xyz = get_euler_xyz_tensor(self.base_quat)
        self.feet_pos = self.rigid_state.view(self.num_envs, self.num_bodies, 13)[:, self.feet_indices, 0:3]
        self.feet_vel = self.rigid_state.view(self.num_envs, self.num_bodies, 13)[:, self.feet_indices, 7:10]
    ...
    def _create_envs(self):
        ...
        self.trot=torch.zeros(1, dtype=torch.long, device=self.device, requires_grad=False)
    ...
    def _get_heights(self, env_ids=None):
        ...
        return heights.view(self.num_envs, -1) * self.terrain.cfg.vertical_scale #num_envs,187(x point * y point)
    ...
    def _reward_feet_slide(self):
        ...
        # 获取足部在XY平面的速度（横向速度）
        foot_vel_xy = self.feet_vel[:, :, :2]  # 只取X和Y方向的速度
        foot_vel_magnitude = torch.norm(foot_vel_xy, dim=-1)
        
        # 惩罚：当足部接触地面且有横向速度时
        # 使用平方惩罚，使得大速度滑动有更严重的惩罚
        penalty = torch.sum(foot_vel_magnitude * contacts, dim=1)
